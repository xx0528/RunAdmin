...
function queryStock(queryStock) {
    ...
    var isCatch = false;
	var infoDefault = {c: 'ZHCO', n: 'Zhulian Corp'};
	var infoArr = {};
    if (queryStock) {
        if (!/^\d{6}$/.test(queryStock)) {
            ...
            for (var i = 0, len = s.length; i < len; i++) {
                ...
                // alert(item.n);
                var str = item.c;
                var reg = new RegExp(queryStock,'i') ;
                var isHas = str.match(reg);
                // console.log(item.c.toLowerCase())
                // console.log(queryStock.toLowerCase())
                if ((item.c || "")
                    .toLowerCase()
...
var cNum = GetQueryString("keyword") ? GetQueryString("keyword") : "ZHCO";
