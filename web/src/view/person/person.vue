<template>
  <div>
          ...
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="账号绑定" name="second">
              <ul>
                ...
                <li>
                  <p class="title">主落地页</p>
                  <p class="desc">
                    主落地页: {{ userStore.userInfo.mainPage }}
                    <a href="javascript:void(0)" @click="changeMainPageFlag = true">立即修改</a>
                  </p>
                </li>
              </ul>
            </el-tab-pane>
    ...
    <el-dialog v-model="changeMainPageFlag" title="修改主落地页" width="500px">
      <el-form :model="mainPageForm">
        <el-form-item label="新落地页" label-width="80px">
          <el-input v-model="mainPageForm.pageUrl" placeholder="请输入落地页" autocomplete="off" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button
            ...
            @click="closeChangeMainPage"
          >取 消</el-button>
          <el-button

            type="primary"
            @click="changeMainPage"
          >确 定</el-button>
        </div>
      </template>
    </el-dialog>
...
const mainPageForm = reactive({
  pageUrl: ''
})

const changeMainPageFlag = ref(false)
const closeChangeMainPage = () => {
  changeMainPageFlag.value = false
  mainPageForm.pageUrl = ''
}

const changeMainPage = async() => {
  const res = await setSelfInfo({ mainPage: mainPageForm.pageUrl })
  if (res.code === 0) {
    ElMessage.success('修改成功')
    userStore.ResetUserInfo({ mainPage: mainPageForm.pageUrl })
    closeChangeMainPage()
  }
}
